...
from cogeo_mosaic.backends.dynamicrtree import DynamicRtreeBackend
...
def MosaicBackend(input: str, *args: Any, **kwargs: Any) -> BaseBackend:
    ...
    # `retree+https://{hostname}/{path}`
    elif parsed.scheme and parsed.scheme.startswith("rtree"):
        input = input.replace("rtree:", "")
        return DynamicRtreeBackend(input, *args, **kwargs)
